...
def upgrade() -> None:
    ...
    if not _has_table('health_metrics'):
    ...
    if _has_table('playing_with_neon'):
        op.drop_table('playing_with_neon')

    if _has_table('chat_messages') and not _has_column('chat_messages', 'message_metadata'):
        op.add_column('chat_messages', sa.Column('message_metadata', sa.JSON(), nullable=True))
    if _has_table('chat_sessions') and not _has_column('chat_sessions', 'last_active_at'):
        op.add_column('chat_sessions', sa.Column('last_active_at', sa.DateTime(), nullable=True))
    if _has_table('users') and not _has_column('users', 'onboarding_completed'):
